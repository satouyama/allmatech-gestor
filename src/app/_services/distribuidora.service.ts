...
@Injectable()
export class DistribuidorasServices {
    ...
    cadastrarDistribuidora(revenda) {
        let body = {
            nome: revenda.nome,
            email: revenda.email,
            cnpj: revenda.cnpj,
            senha: revenda.senha,
            gerenteID: revenda.gerenteID,
        ...
        if (this.user.role == 'gerente') {
            body.gerenteID = this.user.ID;
        }

        return this.http.post(this.ENDPOINT + '/admin/distribuidoras/', body, Config.getHeader());
    }
